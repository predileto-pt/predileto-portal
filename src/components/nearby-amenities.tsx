...
export function NearbyAmenities({ counts, dict }: NearbyAmenitiesProps) {
  ...
  return (
    <div>
      <h3 className="text-[10px] text-gray-400 uppercase mb-2">{d.nearbyAmenities}</h3>
        ...
        {visible.map((key) => (
          ...
          >
            ...
            <div className="min-w-0">
              <p className="text-[11px] font-medium">{counts[key]}</p>
              <p className="truncate text-[10px] text-gray-400">{d[key]}</p>
            </div>
          </div>
        ))}
      </div>
